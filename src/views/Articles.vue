<template>
    <div class="container">
        <v-list lines="three">
            <v-list-item v-for="(item, i) in items" :key="i" :title=item.title :subtitle=item.subtitle
                @click="openModal(item.embedCode)" />
        </v-list>
    </div>

    <!-- 使用 FloatingModal 组件 -->
    <FloatingModal :content="currentEmbedCode" :isVisible="isModalVisible" @close="closeModal" />
    ...
    data() {
        return {
            ...
            items: [
                ...
                {
                    title: '【转载】【齐奕】医疗建筑计算性设计方法前沿分析',
                    value: 5,
                    subtitle: "手术部作为医疗建筑的核心部门，建设成本和使用成本较高，因此其空间效率的提升是业主与建筑师关注的重要问题，其设计过程受到手术室数量、类型、部署位置、建设工艺、管理模式、柱网关系等多方面因素影响，且在设计过程中往往存在多方案比选的过程。手术部设计需满足严格的医疗工艺要求，手术区（各类手术间及其辅助空间）是手术部中对医疗工艺要求最为严格的区域，具有较强的功能性与规则性。",
                    embedCode: "https://1drv.ms/b/s!AlDXLATEEYkClOwxvYRYyG_L7TDWAw?embed=1&em=2"
                }, 
                {
                    title: '【转载】【温宇坤】参数化辅助设计在住院楼标准层设计中的应用',
                    value: 5,
                    subtitle: "手术部作为医疗建筑的核心部门，建设成本和使用成本较高，因此其空间效率的提升是业主与建筑师关注的重要问题，其设计过程受到手术室数量、类型、部署位置、建设工艺、管理模式、柱网关系等多方面因素影响，且在设计过程中往往存在多方案比选的过程。手术部设计需满足严格的医疗工艺要求，手术区（各类手术间及其辅助空间）是手术部中对医疗工艺要求最为严格的区域，具有较强的功能性与规则性。",
